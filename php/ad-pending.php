                    ...
                    <form method="POST" style="display: inline;">
                        ...
                        <div class="mb-3">
                            <select name="subjectExpertise[]" required class="form-select" multiple style="min-width: 150px;">
                                <?php
                                // Query the subjects table to get available subjects
                                $subjects_query = "SELECT subject_name FROM subjects";
                                $subjects_result = mysqli_query($conn, $subjects_query);

                                if ($subjects_result) {
                                    // Get current expertise as an array
                                    $subjectExpertise = explode(",", $row['subjectExpertise']); 

                                    while ($subject_row = mysqli_fetch_assoc($subjects_result)) {
                                        $subject_name = htmlspecialchars(trim($subject_row['subject_name']));
                                        $is_selected = in_array($subject_name, $subjectExpertise) ? "selected" : ""; // Check if the subject is selected
                                        echo "<option value=\"$subject_name\" $is_selected>$subject_name</option>";
                                    }
                                } else {
